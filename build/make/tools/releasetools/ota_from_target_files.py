...
Usage:  ota_from_target_files [flags] input_target_files output_ota_package
  ...
  --no_check_compatibility
      Not check system compatibility, used in userdebug version.
      add by wangwq14. ZUIO-6607

  --zuk_mode  Add file type.txt into update.zip.
      For some purposes, we just want some specially people get a particular
      ota package by online, and not allow to share with others.
      When updater(com.zui.ota) finsih package download, will calculate
      a data with phone serial number(Unique) and package's MD5,
      store it in partition 'cache'. Recovery add a checkpoint,
      to check the ota package source.
      We use parma '--zuk_mode' to add file type.txt to package, to
      enable the package source check.
      If not add type.txt to package, recovery will check where the
      update.zip come from.
      If add type.txt, recovery will skip this check for inner test.
      This parma is ONLY used in NON-A/B mode.
      add by zuk wangwq14. OAUSTINB-211
"""
...
OPTIONS.no_check_compatibility = False
OPTIONS.zuk_mode = False
...
""" % bcb_dev)
  ...
  # add by zuk wangwq14
  if OPTIONS.zuk_mode:
    common.ZipWriteStr(output_zip, "type.txt", "1_DEV_")
...
def WriteBlockIncrementalOTAPackage(target_zip, source_zip, output_zip):
  ...
  device_specific = common.DeviceSpecificParams(
      ...
      input_version=OPTIONS.target_info_dict["recovery_api_version"],
...
""" % bcb_dev)
  ...
  # add by zuk wangwq14
  if OPTIONS.zuk_mode:
    common.ZipWriteStr(output_zip, "type.txt", "0_DEV_")

  # Do device-specific installation (eg, write radio image).
  if OPTIONS.full_radio:
    print("Use full radio in incremental Ota package")
    device_specific.FullOTA_InstallEnd()
  else:
...
def WriteABOTAPackageWithBrilloScript(target_file, output_file,
  ...
  if HasVendorPartition(target_zip):
    ...
    # Add by wangwq14 to disable compatibility check
    if not OPTIONS.no_check_compatibility:
      target_zip = zipfile.ZipFile(target_file, "r")
...
def main(argv):
  ...
  def option_handler(o, a):
    ...
    elif o == "--no_check_compatibility":
      OPTIONS.no_check_compatibility = True
    elif o in ("--zuk_mode",):
      OPTIONS.zuk_mode = True
    else:
      return False
    return True
