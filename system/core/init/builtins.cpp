...
static int do_write_factory_reset(const std::vector<std::string>& args) {
    LOG(INFO) << "add factory reset";
    int fd = open("/dev/block/bootdevice/by-name/misc", O_RDWR);
    if (fd < 0) {
        LOG(INFO) << "add factory reset----------fd <0";
        return -1;
    }
    struct recovery re = {
        "0",
    ...
    WriteFully(fd, &re, sizeof(struct recovery));
    close(fd);
    return 0;
}
//Huaqin modify for factory_reset in shipping image by zhouyingchen at 20180417 for bsp/rom end
...
const BuiltinFunctionMap::Map& BuiltinFunctionMap::map() const {
    ...
    static const Map builtin_functions = {
        ...
        //Huaqin modify for factory_reset in shipping image by zhouyingchen at 20180417 for bsp/rom start
        {"write_factory_reset",     {0,     0,    do_write_factory_reset}},
        //Huaqin modify for factory_reset in shipping image by zhouyingchen at 20180417 for bsp/rom end
