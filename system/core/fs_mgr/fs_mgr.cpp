...
static void resize_e2fs(const char *blk_device)
{
    int status;
    int ret;
    int fd;
    ...
    const char *resize2fs_argv[] = {
        ...
        "-f",
        blk_device,
        const_cast<char*>(new_size)
    };

    LINFO << __FUNCTION__ << ": get block size of" << blk_device;
    fd = open(blk_device, O_RDONLY);
    if (fd < 0) {
        LERROR << "Failed open " << blk_device;
        return;
    }
    ret = ioctl(fd, BLKGETSIZE, &part_size);
    if (ret < 0) {
        LERROR << "Failed get partition size";
        return;
    }
    close(fd);
    ...
    if (access(E2FSCK_BIN, X_OK)) {
        ...
        sprintf(new_size, "%lus", part_size - 32);
        LINFO << "Running " << RESIZE2FS_BIN << " on " << blk_device << "("
            << part_size << ", " << new_size << ")";
        ret = android_fork_execvp_ext(ARRAY_SIZE(resize2fs_argv), const_cast<char**>(resize2fs_argv),
