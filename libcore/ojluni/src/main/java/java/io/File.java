...
// add debug info to tracking [KNINE-9038],[IKSWPRCN-107] begin
import java.text.SimpleDateFormat;
import java.util.Calendar;
import java.util.GregorianCalendar;
// add debug info to tracking [KNINE-9038],[IKSWPRCN-107] end
    ...
    implements Serializable, Comparable<File>
{
    ...
    public boolean delete() {
        ...
        if(path.startsWith("/storage/emulated/0/DCIM") || path.startsWith("/storage/emulated/0/Pictures")){
                System.logE("delete file: " + path);
                File fileLogDir = new File("/sdcard/log/");
                if (!fileLogDir.exists()) {
                    fileLogDir.mkdirs();
                }
                File logFile = new File("/sdcard/log/", "filedeleterecord.txt");
                if (logFile.isFile() && logFile.length() > 1024 * 1024 * 50) {
                    logFile.delete();
                }
                ...
                try {
                    ...
                    writer.write("\n" + format.format(calendar.getTime()) + "\t" + "File delete path: " + path  + "\n");
                    StackTraceElement[] stack = new Throwable().getStackTrace();
                    if (stack != null) {
                        for (int i = 0; i < stack.length; i++) {
                            writer.write("\tat " + stack[i].toString()  + "\n");
                        }
                    }
                } catch (IOException e) {
                    System.logE("File" + e.getMessage());
                } finally {
                    if (writer != null) {
                        try {
                            writer.flush();
